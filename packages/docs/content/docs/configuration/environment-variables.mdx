...
| `USENET_VERIFY_CONTENT`            | Verify Content            | boolean  | `false`      | After the article audit passes, read the first few kilobytes of each video file and check it really is the container its name claims. The audit only proves the articles exist — this catches a release that was posted or assembled wrong. Costs about one article per file, at import and on every recheck.                                                                                                                                                                                |
