...
The Jellyfin compatibility layer on top of upstream AIOStreams. Upstream's own changelog is in `CHANGELOG.md`. Releases are tagged `jf-v<upstream version>-<n>`; the matching Docker image is `ghcr.io/tweakapps/aiostreams-jf:<tag>` (multi-arch: amd64, arm64). `ghcr.io/tweakapps/aiostreams-jf:release` tracks the `release` branch.
